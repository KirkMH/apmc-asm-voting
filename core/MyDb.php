...
class MyDb
{
    ...
    foreach ($dataArray as $key => $value) {
      ...
      $sql = MyDb::run("INSERT INTO {$table} SET {$data}");
      return $sql;
    ...
    public static function update($table, $filter, $id, array $dataArray){
      ...
      $data = implode(', ', $data);
      $sql = MyDb::run("UPDATE {$table} SET {$data} WHERE {$filter} = {$id}");
      return $sql;
    ...
    public static function delete($table, $filter, $id){
      ...
      $sql = MyDb::run("DELETE FROM {$table} WHERE {$filter} = {$id}");
      return $sql;
    ...
    public static function select_all($data, $table, $where = ''){
      ...
      $sql = MyDb::run("SELECT {$data} FROM {$table} WHERE {$where}");
    ...
    public static function select($data, $table, $where = ''){
      ...
      $sql = MyDb::run("SELECT {$data} FROM {$table} WHERE {$where}")->fetch();
    ...
    public static function select_one($field, $table, $where = ''){
      ...
      $sql = MyDb::run("SELECT $field FROM {$table} WHERE $where")->fetchColumn(); 
    ...
    public static function checkCount($table, $where = ''){
         ...
         $sql = MyDb::run("SELECT COUNT(*) FROM {$table} WHERE {$where}")->fetchColumn();  /* Check the number of rows that match the SELECT statement */
  ...
  //users log  
   public static function userlog($date, $time, $action, $user, $activityFrom)
    { 
        ...
        $sql = MyDb::insert('tbl_userlog', $data);
    }
   ...
   public static function logTrail($action, $table, $spec){
    ...
    public static function check_password($password, $field, $table, $where) {
      ...
      $passnya = MyDb::select_one($field, $table, $where);
      $hasher = new PasswordHash(8, TRUE);
      // compare plain password with hashed password
      if ($hasher->CheckPassword( $password, $passnya )) {
        return MyDb::select("*", $table, $where);
      }
      else {
        return false;
      }
