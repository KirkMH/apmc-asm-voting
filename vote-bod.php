...
// $s_code = MyDb::select_one("security_code", "96h3PJ3_users", "ID = " . $_SESSION['user_no']);
// if (is_null($s_code)) {
  ...
  <div id="top" align="center"> <span class="h1w">Election of Board of Directors</span><br>
	  ...
	  <div id="q1" class="toggleDiv" style="display: hide;">
	    <div id="box" align="center"> 
	      ...
	      <p style="text-align: left; font-size: 13pt; font-weight: normal;">You can give up to a maximum of <?=15*$_SESSION['points']?> points. If you want to grant your full shares evenly to your selected candidates, please <a href="vote-bod2.php">click here</a>.</p>
	      ...
	      ?>
			  ...
			  <div class="col-md-4 mb-md-0 p-md-4">
			    <img class="img-fluid img-thumbnail mr-3" src="imageView.php?image_id=<?=$value["candidate_id"]?>" alt="<?=$value['display_name']?>" style="height:auto; width:250px;">
			  </div>
	  ...
	  <div id="q2" class="toggleDiv" style="display: none;" align="center">
	    <div id="box"> <span style="color:red"><b>Summary </b></span>
	        <p class="lead" align="left" style="padding: 30px 30px 10px; "><?=$_SESSION['name']?>, you have selected the following candidates as your directors. Note that you can give up to a maximum of <?=15*$_SESSION['points']?> points. Please click on <kbd style="background-color: rgb(23, 162, 184); color: white;">Submit</kbd> to confirm your vote.<br><br></p>
...
$(document).ready(function(){
   ...
   $('.show_hide').showHide({			 
	...
	$('.vote').on("keyup", function() {
		...
		var max = 15 * <?=$_SESSION['points']?>;
		var total = get_total_votes();
    console.log(total);
	...
	$('.vmore').on("click", function() {
		...
		if (cdt == 15 && vote == 0) {
			$('#prompt_text').html("You have already selected 15 candidates. You may still adjust your selection by zeroing the points of other candidates.");
			$("#myPrompt").modal('show');
		}
		else if (total < max) {
			vote = vote+1;
	...
	$('#submitBtn').on("click", function() {
		...
		if (total == max) {
			...
			$('#prompt_text').html("You still have " + (max - total) + " point(s). Do you want to continue submitting your votes? Click OK to continue, Close to review your votes.");
			$("#promptOK").show();
			$("#myPrompt").modal('show');
